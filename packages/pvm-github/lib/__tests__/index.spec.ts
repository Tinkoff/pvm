...
(env.PVM_GITHUB_TEST_REPO_TOKEN ? describe : describe.skip)('pvm-github', () => {
  describe('releases and tags', () => {
    ...
    it('getRelease', async () => {
      ...
      const [_code, release] = await githubPlatform.getRelease('test')
      expect(release).toMatchObject({
        name: 'test-release',
        description: 'release description',
      })
...
async function prepareRepository() {
  ...
  const githubPlatform = new GithubPlatform({
    ...
    accessToken: env.PVM_GITHUB_TEST_REPO_TOKEN,
    repoPath: {
      owner: GITHUB_TEST_OWNER,
      repo: repoName,
    },
...
async function createWorkflowedPullRequest(repoName: string, repo: any, push) {
