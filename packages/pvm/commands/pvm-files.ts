...
export default (di: Container) => ({
  command: 'files',
  description: 'Output files by glob in the list of packages (packages are choosing by pkgset strategies logic)',
  ...
  handler: async function main(argv): Promise<void> {
    (await getFiles(di, argv.files, { ...argv, ...argv.strategyOption }))
