...
import { debug, log, logger } from '@pvm/core/lib/logger'
...
import { Repository } from '@pvm/repository/lib/repository'
import { UpdateReasonType, UpdateState } from './update-state'
import { vcsInitForUpdate } from './vcs-init'
import getConfig from '@pvm/core/lib/config/get-config'
import getTemplateEnv from '@pvm/template'
...
async function generateNotes(updateState: UpdateState): Promise<void> {
  ...
  for (const pkg of packages) {
    ...
    if (commits && commits.length) {
      debug(`(${pkg.name}) generate release notes`)

      const md = await hostApi.commitsToNotes(commits, pkg)
      debug(`release notes for ${pkg.name}:\n${md.substr(0, 100)}..`)
