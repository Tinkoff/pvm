...
import __shell, { bindToCwd } from '@pvm/core/lib/shell'
import __runShell from '@pvm/core/lib/shell/run'
import __execShell from '@pvm/core/lib/shell/exec'
import { mkdirp, escapeFilePath } from '@pvm/core/lib/fs'
...
import { addTag, getCurrentBranchIgnoreEnv, gitFetch } from '@pvm/core/lib/git/commands'
import { getHostApi } from '@pvm/core/lib/plugins/index'
import { getGitVersion } from '@pvm/core/lib/runtime-env/versions'

import type { AbstractVcs, AddTagOptions, CommitResult, PushOptions, CommitOptions } from '@pvm/vcs/types'
  ...
  const gitVcs: AbstractVcs<GitCommitContext> = {
    ...
    async push(opts: PushOptions = {}) {
      ...
      if (!env.PVM_DIRECT_GIT_PUSH && env.CI && config.git.push.try_load_ssh_keys) {
        ...
        const gitLoadPushCreds = path.resolve(__dirname, '../ci/git-load-push-creds.sh')
