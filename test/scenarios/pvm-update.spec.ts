...
import { UpdateReasonType } from '@pvm/pvm/mechanics/update/update-state'
import revParse from '@pvm/pvm/lib/git/rev-parse'
...
import type { ReleaseData } from '@pvm/pvm/mechanics/releases/types'
...
describe('pvm/update', () => {
  ...
  it.concurrent('update reason for always_changed_workspaces', async () => {
    ...
    const releaseList = JSON.parse(repo.readFile('releaseList.json')) as ReleaseData[]
...
`)
  ...
  describe('packages rename when unified grouping enabled', () => {
    async function commonChangePreparation(repo: RepoTestApi) {
