...
describe('pvm/pkgset', () => {
  ...
  describe('strategy/affected', () => {
    async function testAffected(config, touched = [], output = []) {
      const repo = await initRepo('mono-pkgset-affected', config.constructor === Object ? config : {
        pkgset: {
      ...
      const res = await drainItems(pkgset('affected', {
        cwd: repo.dir,
        f: '%n',
